<template>
  <div style="height: 800px">
    ...
    >
      ...
      <!--列表操作按钮插槽-->
      <template #applyNumSlot>
        <el-table-column align="center" label="单据编号" width="300">
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="交接人" prop="handoverStaffId">
          <el-input v-show="false" v-model="form.handoverStaffId"/>
...
// 页面表单的基本数据
import mixinTableData from "@/views/workhandover/mixins/applyfolwmixins.js";
// 数据接口
import {abandon, editAll} from "@/api/workhandover/applyFlow";
...
export default {
  ...
  methods: {
    detail(v) {
      let path = {
        path: "/workFlow/flowList/" + v.flowId + "/1",
        query: {flowName: v.flowName, flowId: v.flowId, tabIndex: 1},
      };
      this.proxyRouter.push(path);
    },
    handleNodeClick(v) {
    ...
    getSelectTableData(rows) {
      ...
      this.selection = rows;
      rows.map((item) => {
        this.ids.push(item.applyId.toString());
      });
    ...
    connectFun(v) {
      if (v !== undefined) {
        this.ids = [];
        this.ids.push(v.applyId.toString());
      }
      this.open = true;
    },
    // 表单重置
    ...
    abandon(row, c) {
      ...
      if (row === undefined) {
        ...
        ids.push(row.applyId.toString());
      }
      abandon(ids.join(",")).then((response) => {
        this.$message({
          message: "成功",
          type: "success",
        });
