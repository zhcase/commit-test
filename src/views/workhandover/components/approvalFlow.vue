...
// 页面表单的基本数据
import mixinTableData from "@/views/workhandover/mixins/approvalFlowmixins.js";
// 数据接口
import {editAll} from "@/api/workhandover/approvalFlow";
...
export default {
  ...
  methods: {
    detail(v) {
      let path = {
        path: "/workFlow/flowList/" + v.flowId + "/4",
        query: {flowName: v.flowName, flowId: v.flowId, tabIndex: 4},
      };
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        ...
        if (valid) {
          let data = {
            params: {
              handoverStaffId: this.form.handoverStaffId.toString(),
